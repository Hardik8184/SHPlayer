...
//                sortOrder = MediaStore.Audio.Media.DATE_ADDED + " desc";
        ...
        try {
            if (cursor != null && cursor.getCount() >= 1) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (new File(cursor.getString(cursor.getColumnIndex(FavoritePlayTableHelper.PATH))).exists()) {
                        ...
                        System.out.println(" most play path --> " + cursor.getString(cursor.getColumnIndex(FavoritePlayTableHelper.PATH)));
                    }
                }
            }
            closeCrs();
    ...
    public static void runOnUIThread(Runnable runnable, long delay) {
        if (delay == 0) {
            ApplicationSHPlayer.applicationHandler.post(runnable);
        } else {
            ApplicationSHPlayer.applicationHandler.postDelayed(runnable, delay);
        }
    }
