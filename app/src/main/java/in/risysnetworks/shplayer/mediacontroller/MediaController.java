 ...
 */
package in.risysnetworks.shplayer.mediacontroller;
...
import in.risysnetworks.shplayer.ApplicationSHPlayer;
import in.risysnetworks.shplayer.databasemanager.FavoritePlayTableHelper;
import in.risysnetworks.shplayer.databasemanager.MostAndRecentPlayTableHelper;
...
public class MediaController implements NotificationManager.NotificationCenterDelegate, SensorEventListener {
    ...
    public boolean playAudio(SongDetail mSongDetail) {
        ...
        if (MusicPreferance.playingSongDetail != null) {
            Intent intent = new Intent(ApplicationSHPlayer.applicationContext, MusicPlayerService.class);
            ApplicationSHPlayer.applicationContext.startService(intent);
        } else {
            Intent intent = new Intent(ApplicationSHPlayer.applicationContext, MusicPlayerService.class);
            ApplicationSHPlayer.applicationContext.stopService(intent);
        }

        storeResendPlay(ApplicationSHPlayer.applicationContext, mSongDetail);
